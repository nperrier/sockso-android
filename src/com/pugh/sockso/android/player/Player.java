	...
	private void scrobblerMetaChanged() {
		...
		if (entry != null) {
			...
			if (scrobblerApp.equalsIgnoreCase("lastfm")) {
				...
				i.putExtra("duration", entry.getTrack().getDuration() * 1000); // duration
																				// in
																				// milliseconds
				sendBroadcast(i);
				...
				i.putExtra("state", 0); // state 0 = START - track has started
										// playing
				i.putExtra("artist", entry.getAlbum().getArtistName());
				i.putExtra("track", entry.getTrack().getName());
				i.putExtra("duration", entry.getTrack().getDuration()); // duration
																		// in
																		// seconds
				i.putExtra("album", entry.getAlbum().getName());
